...
from . import default_predictive_model_configurations
...
class ConfigParser(Logger):
    ...
    def _parse_predictive_model(self, provided_settings):
        self.predictive_model = Configuration('predictive_model')
        for general_key, general_value in default_predictive_model_configurations.items():
            if general_key in provided_settings:
                general_value = provided_settings[general_key]
            if general_value in ['True', 'False']:
                general_value = general_value == 'True'
            self.predictive_model.add_attr(general_key, general_value)
                ...
                if 'category_details' in setting:
                    ...
                    if not 'descriptors' in setting:
                        ...
                        if self.general.auto_desc_gen:
                            self.log('Automatic descriptor generation requested, but no descriptors provided for parameter "%s". Will not generate descriptors for this parameter. This message will only be shown once.' % setting['name'], 'WARNING')
                num_cats = len(setting['options'])
            else:
            ...
            for setting in provided_settings:
                for key in obj_configs.keys():
                    if key not in setting:
                        self.log(f'you need to define "{key}" in multi-objective optimizations', 'FATAL')
    ...
    def settings(self):
        ...
        settings_dict['predictive_model'] = self.predictive_model.to_dict()
    ...
    def get_pred(self, attr):
        return self.predictive_model.get_attr(attr)
    ...
    def _parse(self, config_dict):
        ...
        if 'predictive_model' in self.config:
            self._parse_predictive_model(self.config['predictive_model'])
        else:
            self._parse_predictive_model({})
