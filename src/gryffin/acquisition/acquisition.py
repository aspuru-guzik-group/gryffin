...
class Acquisition(Logger):
	...
	def _proposal_optimization_thread(self, proposals, kernel_contribution, kernel_contribution_feas, unfeas_frac,
		...
		def kernel(x):
			...
			if self.predictive_model.is_trained:
				features = self.training_set_gen.construct_features(x) # construct features
				g, _ = self.predictive_model.predict(features, return_unscaled=True) # make prediction
				pearson_coeff = self.predictive_model.get_pearson_coeff() # get pearson coeff
				acq_samp = (num + self.sampling_param_values[batch_index] + pearson_coeff*g*self.inverse_volume) * inv_den / (1 + self.inverse_volume*inv_den)
			else:
	...
	def propose(self, best_params, kernel_contribution, kernel_contribution_feas, unfeas_frac, sampling_param_values,
				predictive_model, training_set_gen, inverse_volume, num_samples=200, dominant_samples=None):
